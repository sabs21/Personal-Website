...
const Sunset = () => {
    ...
    const CameraWobble = () => {
        useFrame((state) => {
            state.camera.position.x = state.camera.position.x + Math.sin(time * 3) / 80;
            state.camera.position.y = state.camera.position.y + Math.sin(time * 1.4) / 90;
            state.camera.position.z = state.camera.position.z + Math.cos(time * 2) / 70;
    ...
    return (
        <div>
            <Canvas colorManagement shadows shadowMap camera={{ zoom: 80, position: [50, 15, 100] }}>
